...
Процедура ПриСозданииНаСервере(Форма, ПараметрыРазмещения = Неопределено) Экспорт
	...
	ВывестиКоманды(Форма, Команды, ПараметрыРазмещения);
		...
		// Начало доработки
	// Бакуменко С.В. Задача от 26.04.2023 16:59:44 
	// Описание
	// Исходный код
	//
	//
	...
	Если Прав(Форма.ИмяФормы,11) = "ФормаСписка"  Тогда
		
		БС_РаботаСФормойСервер. ПриСозданииНаСервереФормаСписка(Форма);	
		
	ИначеЕсли  Прав(Форма.ИмяФормы,14) = "ФормаДокумента" Тогда
		
		БС_РаботаСФормойСервер.ПриСозданииНаСервереФормаДокумента(Форма);
